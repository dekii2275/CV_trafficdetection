...
class AnalyzeOnRoadForMultiprocessing():
    ...
    def run_multiprocessing(self):
        ...
        if self.show_log:
            log_process = Process(target=log, args=(self.names, self.shared_data))
            log_process.start()
            self.processes.append(log_process)  
    ...
    def join_process(self):   
        ...
        """Join các process cho mode chạy script (livestream thì để tự chạy đến khi Ctrl+C)."""
        for p in self.processes:
            p.join()  
        print("All processes stopped.")
